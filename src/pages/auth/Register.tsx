...
import { ArrowLeft01Icon, CheckmarkCircle02Icon, CircleIcon, GoogleIcon } from '@hugeicons/react';
...
export function Register() {
  ...
  const googleLogin = useGoogleLogin({
    ...
    onSuccess: async (codeResponse) => {
      setIsLoading(true);
      try {
        const response = await authService.directGoogleSignIn(codeResponse.code, initialRole);
        if (response.success && response.user) {
          ...
          setErrors({ general: response.error?.message || 'Google Sign-Up failed.' });
        }
      } catch (err) {
                  ...
                  >
                    {step === 0 && (
                      <>
                        ...
                        />
                        {errors.general && (
                          <div className="flex flex-row items-center gap-2 bg-[#FFEBEE] rounded-lg p-3 border border-[#FFCDD2] -mt-2">
                ...
                {step === 0 && (
                  <>
                    <Button
                      type="button"
                      variant="outline"
                      className="w-full !border-border-light text-text-primary !py-4 rounded-[50px] flex items-center justify-center gap-2 mt-2"
                      onClick={() => googleLogin()}
                    >
                    ...
                    <button onClick={() => navigate('/login')} className="flex justify-center mt-6">
